function save_preferences() {
  ...
  localStorage.setItem(
    "caffeine_excess",
    document.getElementById("caffeine_excess").checked
  );
}
...
function generate_schedules() {
  ...
  var depart_tz = Math.floor(
    parseFloat(localStorage.getItem("depart_city_lng")) / 15
  );
  var arrive_tz = Math.floor(
    parseFloat(localStorage.getItem("arrive_city_lng")) / 15
  );
  ...
  function schedule(index, change, sleep_start, sleep_end, lbt) {
    ...
    for (const day of Array(index).keys()) {
      ...
      temp.push([
        sleep_start,
        sleep_end,
        revert(sleep_start + delta),
        revert(sleep_end + delta)
      ]);
    }
    //add score to temp
  ...
  function score(days_before, change, coffee, alcohol, travel_sleep) {
    return (
      100 -
      (Math.abs(delta) +
        (change / days_before) * 9 +
        coffee * 0.5 +
        alcohol * 0.7 +
        travel_sleep * 0.5)
    );
  ...
  //2nd
  schedule(delta, 1, sleep_start, sleep_end, lbt); // this should add another schedule this should be the ideal schedule
  //3rd
  schedule(delta / 2, 2, sleep_start, sleep_end, lbt); // this should add another schedule not as good as above but is more reasonable
